...
/**
 * Razred predstavlja igralno polje, tj. mrežo kvadratkov, na katere postavimo
 * ploščice s številkami. Poskrbi za grafični prikaz uporabniškega vmesnika ter
 * za interakcijož z igralcem. Igralec lahko pritisne smerne tipke za premikanje
 * ploščic po igralnem polju. Tipka ESC zaključi igro, tipka ENTER pa začne novo
 * igro. Za logiko igre poskrbi razred Logika, katerega metode kličemo pri
 * grafičnem prikazu igre (ob odzivu na pritisnjene tipke in ob izrisu
 * spremenjenega igralnega polja). Ko igralec doseže ciljno ploščico (privzeto
 * 2048), se pod točkami izpiše, da je zmagal, in igra se nadaljuje.
 */
public class IgralnoPolje extends JPanel {
    ...
    private final static int SIRINA = STEVILO_POLJ * (VELIKOST_POLJA + ROB) + ROB; // širina igralne površine (v
                                                                                   // pikslih)
    private final static int VISINA = STEVILO_POLJ * (VELIKOST_POLJA + ROB) + ROB + VELIKOST_GLAVE; // višina igralne
                                                                                                    // površine (v
                                                                                                    // pikslih)
    ...
    public IgralnoPolje() {
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                // zanimajo nas puščice levo, desno, gor in dol ter ENTER (nova igra) in ESC
                // (konec)
    ...
    public void paintComponent(Graphics g) {
        ...
        // nariši polje STEVILO_POLJ x STEVILO_POLJ (privzeto 4 x 4) z vpisanimi
        // številkami
        for (int i = 0; i < STEVILO_POLJ; i++) {
            for (int j = 0; j < STEVILO_POLJ; j++) {
                ...
                g.fillRoundRect(ROB + j * (VELIKOST_POLJA + ROB), VELIKOST_GLAVE + ROB + i * (VELIKOST_POLJA + ROB),
                        VELIKOST_POLJA, VELIKOST_POLJA, LOK, LOK);
                if (ploscica > 0) {
                    ...
                    g.drawString(stevilka, ROB + j * (VELIKOST_POLJA + ROB) + (VELIKOST_POLJA - dolzina) / 2,
                            VELIKOST_GLAVE + ROB + i * (VELIKOST_POLJA + ROB) + VELIKOST_PISAVE[ind]
                                    + (VELIKOST_POLJA - VELIKOST_PISAVE[ind] - ROB) / 2);
                }
            }
        }

        // če je igra končana, izpiši sporočilo ob koncu igre na sredino igralne
        // površine
        if (Logika.jeKonec()) {
